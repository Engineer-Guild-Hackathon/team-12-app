...
def test_config_env_overrides(monkeypatch: pytest.MonkeyPatch):
    ...
    よくある構成を想定：
        ...
        - INLINE_MAX_IMAGE_BYTES / HTTP_TIMEOUT を設定 or 参照
    モジュールの仕様が異なる場合は、存在しない属性の検証はスキップ。
    """
    monkeypatch.setenv("GEMINI_API_KEY", "dummy-key-xyz")
    monkeypatch.setenv("INLINE_MAX_IMAGE_BYTES", "123456")
    ...
    if hasattr(cfg, "INLINE_MAX_IMAGE_BYTES"):
        assert isinstance(cfg.INLINE_MAX_IMAGE_BYTES, int)
    else:
        # 環境変数直読みの実装かもしれない
        pytest.skip("INLINE_MAX_IMAGE_BYTES が属性として無い構成のためスキップ")
