...
class SearchService:
    ...
    def find_related_posts(post_id: uuid.UUID, num_results: int = 5) -> Optional[List[str]]:
        ...
        try:
            client = discoveryengine.SearchServiceClient(client_options=ClientOptions(api_endpoint=API_ENDPOINT))
            ...
            query = discoveryengine.SearchRequest.Query(document=document_name)
