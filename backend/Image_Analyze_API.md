...
> - 画像 MIME はサーバ側でスニフ（`python-magic`）し、**非画像は 400**。空バイトも 400。   
...
- 画像サイズが小さい場合は **インライン（Base64 + JPEG 再エンコード）**、大きい場合は **Files API** でアップロードしてから Gemini を呼びます。応答テキストは JSON としてパースし、**`{title, discovery, question}`** の辞書として返します。  
